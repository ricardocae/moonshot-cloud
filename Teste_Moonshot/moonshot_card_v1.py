...
def _draw_pill(draw, k, left, y_sym, symbol, side, leverage, f_title, f_pill):
    ...
    draw.rounded_rectangle((x_pill, y_pill, x_pill+pill_w, y_pill+pill_h), radius=int(pill_h/2),
                           fill=PILL_LONG if side.upper()=="LONG" else PILL_SHORT)
...
def generate_trade_card(
    ...
    # pill
    _draw_pill(draw, k, left, y_sym, symbol, side, leverage, f_title, f_pill)
